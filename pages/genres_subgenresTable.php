<?php
	...
	$body = new Template("genres_subgenresTable.html"); 		// sottotemplate per la home

	$main->setContent("nickname", $_SESSION['nickname']);

    $query = "SELECT id, genre, subgenre FROM genres_subgenres";
    ...
    //pupolating genres options
    $query = "SELECT name FROM genres";
    $result = $mysqli->query($query);

    while($row = $result->fetch_assoc()){
        $body->setContent("getGenre", $row['name']);
    }

    //pupolating subgenres options
    $query = "SELECT name FROM subgenres";
    $result = $mysqli->query($query);

    while($row = $result->fetch_assoc()){
        $body->setContent("getSubgenre", $row['name']);
    }
