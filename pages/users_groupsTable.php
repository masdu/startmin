<?php
	...
	$body = new Template("users_groupsTable.html"); 		// sottotemplate per la home

	$main->setContent("nickname", $_SESSION['nickname']);

    $query = "SELECT id, user, `group` FROM `users_groups`";
	...
	//populating select groups
	$query = "SELECT name FROM `groups`";

	$result = $mysqli->query($query);
	
	while($row = $result->fetch_assoc()){
		$body->setContent("groups", $row['name']);
	}
